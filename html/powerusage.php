...
$appliance1_rating = 115;
$appliance2_rating = 100;
$appliance3_rating = 50;
$appliance4_rating = 10;
...
foreach ($jsonfile1 as $array){
  ...
  if($ontime > $startday && $offtime < $endday){
    ...
    $time_usage1[$startday] = $time_usage1[$startday] + (((($offtime - $ontime)/3600)*$appliance1_rating)/1000);
  }
  }}}
...
foreach ($jsonfile2 as $array){
  ...
  if($offtime > $ontime){
  if($ontime > $startday && $offtime < $endday){
    $time_usage2[$startday] = $time_usage2[$startday] + (((($offtime - $ontime)/3600)*$appliance2_rating)/1000);
  }}}}
...
foreach ($jsonfile3 as $array){
  ...
  if($offtime > $ontime){
  if($ontime > $startday && $offtime < $endday){
    $time_usage3[$startday] = $time_usage3[$startday] + (((($offtime - $ontime)/3600)*$appliance3_rating)/1000);
  }}}}
...
foreach ($jsonfile4 as $array){
  ...
  if($offtime > $ontime){
  if($ontime > $startday && $offtime < $endday){
    $time_usage4[$startday] = $time_usage4[$startday] + (((($offtime - $ontime)/3600)*$appliance4_rating)/1000);
  }}}}
      ...
      <link rel='icon' type='image/png' href='favicon.png' />
	    ...
	    var data = new google.visualization.DataTable();
	      ...
	      data.addColumn('number', 'Appliance 4');
              data.addColumn('number', 'Appliance Total');
              data.addRows([
<?php
...
foreach($days as $key => $value){
  ...
  if(array_key_exists ($key, $time_usage1)){
    ...
    $days_total = $days_total + $time_usage1[$key];
  } else {
    $daysdata = $daysdata . 0 . ", ";
  }
  ...
  if(array_key_exists ($key, $time_usage2)){
    ...
    $days_total = $days_total + $time_usage2[$key];
  } else {
    $daysdata = $daysdata . 0 . ", ";
  }
  ...
  if(array_key_exists ($key, $time_usage3)){
    ...
    $days_total = $days_total + $time_usage3[$key];
  } else {
    $daysdata = $daysdata . 0 . ", ";
  }
  ...
  if(array_key_exists ($key, $time_usage4)){
    ...
    $days_total = $days_total + $time_usage4[$key];
  } else {
    $daysdata = $daysdata . 0;
  }

  $daysdata = $daysdata ."," . $days_total .  "],";
...
<?php
        echo "<tag><strong><a href='timeusage.php' class='button' style='height:20px;'>> User Usage <</a></strong><br>";
        ...
        <br><br>
	<form action="powerusage.php" method="post">
